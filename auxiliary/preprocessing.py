...
def bootstrap(num_corpus, num_sentences, sampling_size, pathPlots,
              bootstrapping=True, plot=True):
    """
...
def get_spacy_model(language):
    ...
    if language == 'en':
        ...
        sys.exit()
    nlp = spacy.load(spacy_model, disable=['ner', 'parser']) # disabling Named Entity Recognition for speed
...
def sentence_preprocessing(sentences, nlp, lowercase=True, lemmatize=True):
    ...
    """
    if lowercase:        
        remove_non_characters = (re.sub("[^a-zA-Z äöüßÄÖÜôÔèÈéÉàÀÇéâêîôûàèùëïü]+", ' ', row.lower())
                                 for row in sentences)
    else:
        remove_non_characters = (re.sub("[^a-zA-Z äöüßÄÖÜôÔèÈéÉàÀÇéâêîôûàèùëïü]+", ' ', row)
                                 for row in sentences)    
    sent = [text_cleaning(doc, lemmatize) for doc in nlp.pipe(remove_non_characters, n_threads=-1, batch_size=1000)]    
    return sent
...
def plot_word_frequency(sent, pathFolder, filename):
    ...
    # save fdist to csv
    with open(os.path.join(pathFolder, filename + "_fdist.csv"), "w", encoding = 'utf-8-sig', newline='') as fp:
...
def get_bigram_and_trigram_model(sentences, n_gram, language, pathSave):
    ...
    trigram = Phrases(bigram[sentences],
                      ...
                      progress_per=100000)
    print('Time to create bigram and trigram models: {} mins'.format(round((time() - t) / 60, 2)))    
