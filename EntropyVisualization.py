...
for count, name in enumerate(names):
    ...
    for w in entropy_distribution[name].keys():
        word_count.append(
            np.mean(
                [entropy_distribution[name][w][num][1] for num in range(num_corpus)]
            )
        )
        mean_entropy.append(
            np.mean(
                [entropy_distribution[name][w][num][0] for num in range(num_corpus)]
            )
        )
        std_entropy.append(
            np.std([entropy_distribution[name][w][num][0] for num in range(num_corpus)])
        )
    ...
    entropy_dict["samples_" + name] = [
        num_corpus for _ in entropy_distribution[name].keys()
    ]
...
class BasePlot(object):
    ...
    def plot(self):
        ...
        for name in self.names:
            sns.kdeplot(
                self.entropy_dict[self.column + name],
                label=name,
    ...
    def label(self):
        ...
        if self.legend:
            plt.legend(loc="lower right", fontsize=self.fontsize_legend)
...
def boxplot():
    ...
    plt.boxplot(data, positions=positions, labels=names, notch=True)
    ...
    if save:
        fig.savefig(
            os.path.join(pathPlot, "boxplot_entropy_{}.png".format("_".join(names)))
        )
...
def boxplot_ngram():
    ...
    if save:
        fig.savefig(
            os.path.join(pathPlot, "boxplot_ngram_{}.png".format("_".join(names)))
        )
...
def boxplot_count():
    ...
    if save:
        fig.savefig(
            os.path.join(pathPlot, "boxplot_count_{}.png".format("_".join(names)))
        )
...
def plot_histogram():
    ...
    if save:
        fig.savefig(
            os.path.join(pathPlot, "hist_entropy_{}.png".format("_".join(names)))
        )
...
def plot_KDE():
    ...
    for name in names:
        ...
        sns.kdeplot(entropy_dict["mean_" + name], label=name, ls=ls, shade=False)
    ...
    if save:
        fig.savefig(
            os.path.join(pathPlot, "KDE_entropy_{}.png".format("_".join(names)))
        )
...
def plot_ECDF_entropy():
    ...
    for name in names:
        ...
        sns.kdeplot(
            entropy_dict["mean_" + name], label=name, cumulative=True, shade=False
        )
    ...
    if save:
        fig.savefig(
            os.path.join(pathPlot, "ECDF_entropy{}.png".format("_".join(names)))
        )
...
def scatter_entropy_vs_count():
    ...
    if save:
        fig.savefig(
            os.path.join(
                pathPlot, "scatter_entropy_vs_count_{}.png".format("_".join(names))
            )
        )
...
def plot_KDE_2D():
    ...
    for count, name in enumerate(names):
        ...
        if save:
            fig.savefig(
                os.path.join(pathPlot, "KDE_2D_entropy_vs_count_{}.png".format(name))
            )
...
def entropy_to_csv():
    ...
    for count, name in enumerate(names):
        ...
        if save:
            df.to_csv(
                os.path.join(pathCSV, "{}.csv".format(name)),
                sep=";",
                encoding="utf-8-sig",
            )
